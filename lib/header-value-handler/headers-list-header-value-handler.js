 ...
 * normalized using {@link util.headerCase}. If the value is not already in the
 * list, it is added to the list after a comma. The provided value can also be a
 * comma-separated list, in which case each element is processed individually.
 *
...
class HeadersListHeaderValueHandler {
	...
	value(v) {
		...
		String(v).trim().split(/\s*,\s*/).forEach(element => {
			this._value.add(util.headerCase(element))
		});
	}
}
