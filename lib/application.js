...
const NoopAuthenticatorProvider =
	require('./authenticator/noop-authenticator.js');
...
class Application {
	...
	constructor(configFn, logger) {
		...
		this._serviceProviders.set(
			'authenticator', new NoopAuthenticatorProvider());
		this._serviceProviders.set(
			'actor-registry', new NoopActorRegistryProvider());
		this._serviceProviders.set(
			'actor-registry-cache', new ActorRegistryCacheProvider());
		this._serviceProviders.set(
	...
	init() {
		...
		const serviceProviders = this._serviceProviders;
		const serviceInitSequence = new Set();
		const serviceDepsChain = new Set();
		function addToServiceInitSequence(serviceProvider, serviceName) {
			...
			if (deps && (deps.length > 0)) {
				...
				deps.forEach(depServiceName => {
					const depServiceProvider =
						serviceProviders.get(depServiceName);
					if (!depServiceProvider)
						throw new Error(
							'Service "' + serviceName
								+ '" depends on unknown service "'
								+ depServiceName + '".');
					addToServiceInitSequence(depServiceProvider, depServiceName);
				});
		...
		this._serviceProviders.forEach((serviceProvider, serviceName) => {
			...
			addToServiceInitSequence(serviceProvider, serviceName);
		});

		// create runtime
		const services = new Map();
		this._runtime = new Runtime(this._configFn, this._logger, services);

		// instantiate services
		for (serviceName of serviceInitSequence) {
			this._logger.debug(
				'instantiating service %s', serviceName);
			services.set(
				serviceName,
				serviceProviders.get(serviceName).createService(this._runtime));
		}
		...
		// TODO: shutdown initialized so far services if error?
	...
	shutdown() {
		...
		// TODO: disable respond?
		// TODO: implement: shutdown service in reverse order
	}
	...
	respond(httpRequest, httpResponse) {
		...
		// parse request URL and get the request method
		...
		if (!allowedMethods.has(method === 'HEAD' ? 'GET' : method)) {
			...
			return this._sendResponse(
				httpResponse, method, match, response, null, null, requestUrl,
				httpRequest.headers);
		}

		// authenticate the request
		this._runtime.service('authenticator')
	 ...
	 * @param {Object.<string,string>} [httpResponseHeaders] HTTP response
	 * headers to include.
	 */
	_sendErrorResponse(
		...
		httpResponseHeaders['Content-Length'] = Buffer.byteLength(entity);

		httpResponse.writeHead(httpResponseCode, httpResponseHeaders);
	...
	_sendResponse(
		...
		// get authenticator
		const authService = this._runtime.service('authenticator');
	...
	_getResponseEntityDataBuffer(entity) {
		...
		let marshaller = this._runtime.service(
