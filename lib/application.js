...
const JsonMarshaller = require('./marshaller/json-marshaller.js');
...
class Application {
	...
	constructor(configFn, logger) {
		...
		this._serviceProviders.set('marshaller:application/json', {
			createService() {
	...
	respond(httpRequest, httpResponse) {
		...
		if (!allowedMethods.has(method === 'HEAD' ? 'GET' : method)) {
			const allAllowedMethods = new Set(allowedMethods);
			if (allowedMethods.has('GET'))
				allAllowedMethods.add('HEAD');
			allAllowedMethods.add('OPTIONS');
			return this._sendErrorResponse(
				httpResponse, 405, 'X2-405-1',
				'Method not supported by the API endpoint.', {
					'Allow': Array.from(allAllowedMethods).join(', ')
				});
		}
	 ...
	 * @param {string} resourceUri Target resource URI.
	 * @param {string[]} uriParams URI parameters extracted from the resource
	 * URI.
	 * @param {?EndpointHandler} handler API endpoint handler, or
	 * <code>null</code> if no matching handler.
	 ...
	 */
	_sendResponse(
		httpResponse, method, resourceUri, uriParams, handler, response,
		requestActor, responseActor, requestUrl, httpRequestHeaders) {
		...
		if (origin) {
			...
			if (this._allowedPublicOriginsPattern
				&& handler && handler.isPublic(resourceUri, uriParams)
				&& this._allowedPublicOriginsPattern.test(origin)) {
			...
			if (allowedOrigin)
				...
				.concat((handler && handler.exposedResponseHeaders) || [])
				.filter(
		...
		authService.addResponseAuthInfo(
			response, requestActor, responseActor, requestUrl,
			httpRequestHeaders);
		...
		if (response.hasStreams) {
			...
			if (entities.length === 1) {
				...
				if (entity.data instanceof Buffer) {
					...
					buf = marshaller.serialize(entity.data, entity.contentType);
				}
