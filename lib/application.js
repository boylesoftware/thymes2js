...
class Application {
	...
	respond(httpRequest, httpResponse) {
		...
		/*httpRequest.on('timeout', (socket) => {
			console.log('>>> HTTP REQ TIMEOUT');
		});*/
	...
	onTimeout(socket) {
		...
		console.log('>>> in application onTimeout');

		// TODO: check if request has been read completely, send 500 if so
