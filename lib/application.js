...
class Application {
		...
		try {
			...
			if (!match)
				return this._sendResponse(
					...
					(new EndpointCallResponse(this, 404)).entity({
						errorCode: 'X2-404-1',
			...
			if (!this._knownHttpMethods.has(method))
				return this._sendResponse(
					...
					(new EndpointCallResponse(this, 501)).entity({
						errorCode: 'X2-501-1',
			...
			if (!allowedMethods.has(method === 'HEAD' ? 'GET' : method)) {
				const response = new EndpointCallResponse(this, 405);
				...
				.then(authResult => {
					...
					if (!match.handler.isAllowed(
						...
						if (authResult.actor === null)
							return this._sendResponse(
								httpResponse, method, match,
								(new EndpointCallResponse(this, 401)).header(
						...
						return this._sendResponse(
							httpResponse, method, match,
							(new EndpointCallResponse(this, 403)).entity({
								errorCode: 'X2-403-1',
					...
					const ctx = new EndpointCallContext(
						this, this._runtime, authResult.actor);
					...
					Promise.resolve(match.handler.handleCall(ctx)).then(
						result => {
							...
							if (result instanceof EndpointCallResponse)
								...
								response = new EndpointCallResponse(this, 204);
							else
								response = new EndpointCallResponse(this, 200)
									.entity(result);
						...
						err => { // error in the handler
							...
							this._sendResponse(
								httpResponse, method, match,
								(new EndpointCallResponse(this, 500)).entity({
									errorCode: 'X2-500',
	...
	_sendOptionsResponse(httpResponse, match, requestHeaders, allowedMethods) {
		...
		const response = new EndpointCallResponse(this, 200);
	 ...
	 * @param {EndpointCallResponse} response The response object (normally
